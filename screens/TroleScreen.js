...
export default function TroleScreen() {
  const navigation = useNavigation();
  const [trole, setTrole] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch('https://mock.apidog.com/m1/988210-973885-default/trole/tipos')
      .then(res => res.json())
      .then(data => {
        setTrole(data);
        setLoading(false);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.card}>
        <Image source={require('../assets/TroleC1.jpg')} style={styles.image} />
        <Text style={styles.transportName}>Trole</Text>
        <Text style={styles.transportDesc}>El Trole de Quito conecta el sur y norte de la ciudad de Quito de manera rápida y eficiente.</Text>
      ...
      <View style={styles.listSection}>
        <Text style={styles.listTitle}>{trole.length} tipos</Text>
        {loading ? (
