...
export default function EcoviaScreen() {
  const navigation = useNavigation();
  const [ecovias, setEcovias] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch('https://mock.apidog.com/m2/988210-973885-default/18776607')
      .then(res => res.json())
      .then(data => {
        setEcovias(data);
        setLoading(false);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.card}>
        <Image source={require('../assets/ecovia.jpg')} style={styles.image} />
        <Text style={styles.transportName}>Ecovía</Text>
      ...
      <View style={styles.listSection}>
        <Text style={styles.listTitle}>{ecovias.length} tipos</Text>
        {loading ? (
