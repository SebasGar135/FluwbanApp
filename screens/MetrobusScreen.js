...
export default function MetrobusScreen() {
  ...
  const [metrobus, setMetrobus] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch('https://mock.apidog.com/m2/988210-973885-default/18776718')
      .then(res => res.json())
      .then(data => {
        setMetrobus(data);
        setLoading(false);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.card}>
        <Image source={require('../assets/metrobus.jpg')} style={styles.image} />
        <Text style={styles.transportName}>Metrobuses</Text>
        <Text style={styles.transportDesc}>El Metrobús de Quito conecta el sur y norte de la ciudad de Quito de manera rápida y eficiente.</Text>
      ...
      <View style={styles.listSection}>
        <Text style={styles.listTitle}>{metrobus.length} tipos</Text>
        {loading ? (
